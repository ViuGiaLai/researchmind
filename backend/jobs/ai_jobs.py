...
def update_job(
    session, job_id: str, *, status: str | None = None, progress: int | None = None, error: str = ""
) -> AIJob:
...
def cancel_job(session, job_id: str) -> AIJob:
    return update_job(session, job_id, status="cancelled")


def resumable_jobs(session) -> list[AIJob]:
    return (
        session.query(AIJob)
        .filter(AIJob.status.in_(["queued", "running", "retrying"]), AIJob.attempts < AIJob.max_attempts)
        .all()
    )
