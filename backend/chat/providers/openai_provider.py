...
class OpenAIProviderMixin:
    ...
    def _generate_github(
        self, prompt: str, api_key: str, model: str, max_tokens: int = 1024, system_prompt_override: str = None
    ) -> "GenerationResult":
        try:
            ...
            return GenerationResult(
                content=content, citations=citations, model_used=f"github/{model}", finish_reason=finish_reason
            )
            ...
            return GenerationResult(
                content=_t(
                    "provider.error.github_http",
                    lang,
                    status=e.response.status_code,
                    detail=e.response.text[:200] + detail,
                ),
                citations=[],
                model_used="github/error",
                finish_reason="error",
            )
            ...
            logger.error(f"GitHub Models generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.github", lang, error=str(e)),
                citations=[],
                model_used="github/error",
                finish_reason="error",
            )
    ...
    def _generate_groq(
        ...
        try:
            ...
            content = self._verify_citations(content, citations)
            return GenerationResult(
                content=content, citations=citations, model_used=f"groq/{model}", finish_reason=finish_reason
            )
        except httpx.HTTPStatusError as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            ...
            logger.error(f"Groq generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.groq", lang, error=str(e)),
                citations=[],
                model_used="groq/error",
                finish_reason="error",
            )
    ...
    def _generate_nvidia(
        ...
        try:
            ...
            content = self._verify_citations(content, citations)
            return GenerationResult(
                content=content, citations=citations, model_used=f"nvidia/{model}", finish_reason="stop"
            )
        except Exception as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"NVIDIA generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.nvidia", lang, error=str(e)),
                citations=[],
                model_used="nvidia/error",
                finish_reason="error",
            )

    def _generate_freemodel(
        self, prompt: str, api_key: str, model: str, max_tokens: int = 1024, system_prompt_override: str = None
    ) -> "GenerationResult":
        try:
            sp = system_prompt_override or self._get_system_prompt()
            ...
            content = self._verify_citations(content, citations)
            return GenerationResult(
                content=content, citations=citations, model_used=f"freemodel/{model}", finish_reason="stop"
            )
        except Exception as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"FreeModel generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.freemodel", lang, error=str(e)),
                citations=[],
    ...
    def _generate_openrouter(
        ...
        try:
            ...
            content = self._verify_citations(content, citations)
            return GenerationResult(
                content=content, citations=citations, model_used=f"openrouter/{model}", finish_reason="stop"
            )
        except httpx.HTTPStatusError as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            ...
            return GenerationResult(
                content=_t(
                    "provider.error.openrouter", lang, status=e.response.status_code, detail=e.response.text[:200]
                ),
                citations=[],
                model_used="openrouter/error",
                finish_reason="error",
            )
        except Exception as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"OpenRouter generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.openrouter_generic", lang, error=str(e)),
                citations=[],
                model_used="openrouter/error",
                finish_reason="error",
            )
    ...
    def _generate_cohere(
        ...
        try:
            ...
            response = self.http_client.post(
                f"{self.cohere_url}/chat/completions",
                headers=headers,
                json=payload,
                timeout=60.0,
            )
            ...
            content = self._verify_citations(content, citations)
            return GenerationResult(
                content=content, citations=citations, model_used=f"cohere/{model}", finish_reason="stop"
            )
        except httpx.HTTPStatusError as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            ...
            return GenerationResult(
                content=_t(
                    "provider.error.cohere_http", lang, status=e.response.status_code, detail=e.response.text[:200]
                ),
                citations=[],
                model_used="cohere/error",
                finish_reason="error",
            )
        except Exception as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"Cohere generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.cohere", lang, error=str(e)),
                citations=[],
                model_used="cohere/error",
                finish_reason="error",
            )
    ...
    def _generate_cloudflare(
        ...
        try:
            ...
            response = self.http_client.post(
                f"{self.cloudflare_url}/chat/completions",
                headers=headers,
                json=payload,
                timeout=60.0,
            )
            ...
            content = self._verify_citations(content, citations)
            return GenerationResult(
                content=content, citations=citations, model_used=f"cloudflare/{model}", finish_reason="stop"
            )
        except httpx.HTTPStatusError as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            ...
            return GenerationResult(
                content=_t(
                    "provider.error.cloudflare_http",
                    lang,
                    status=e.response.status_code,
                    detail=e.response.text[:200] + detail,
                ),
                citations=[],
                model_used="cloudflare/error",
                finish_reason="error",
            )
        except Exception as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"Cloudflare Workers AI generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.cloudflare", lang, error=str(e)),
                citations=[],
                model_used="cloudflare/error",
                finish_reason="error",
            )
    ...
    def _generate_cerebras(
        ...
        try:
            ...
            content = self._verify_citations(content, citations)
            return GenerationResult(
                content=content, citations=citations, model_used=f"cerebras/{model}", finish_reason="stop"
            )
        except httpx.HTTPStatusError as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            ...
            return GenerationResult(
                content=_t(
                    "provider.error.cerebras_http", lang, status=e.response.status_code, detail=e.response.text[:200]
                ),
                citations=[],
                model_used="cerebras/error",
                finish_reason="error",
            )
        except Exception as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"Cerebras generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.cerebras", lang, error=str(e)),
                citations=[],
                model_used="cerebras/error",
                finish_reason="error",
            )
    ...
    def _generate_deepseek(
        ...
        try:
            ...
            return GenerationResult(
                content=content, citations=citations, model_used=model_name, finish_reason=finish_reason
            )
        except Exception as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"DeepSeek generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.deepseek", lang, error=str(e)),
                citations=[],
    ...
    def _stream_openai(self, prompt: str, api_key: str, model: str, base_url: str, max_tokens: int = 1024):
        ...
        try:
            ...
            with self.http_client.stream(
                ...
                headers=headers,
                json=payload,
                timeout=getattr(settings, "openai_stream_timeout", 60.0),
            ) as response:
                response.raise_for_status()
    ...
    def _stream_deepseek(self, prompt: str, api_key: str, max_tokens: int = 1024, is_free: bool = False):
        ...
        try:
            ...
            is_fast = getattr(self._local, "reasoning_mode", "fast") == "fast"
            with self.http_client.stream(
                "POST", "https://api.deepseek.com/chat/completions", headers=headers, json=payload, timeout=60.0
            ) as response:
                response.raise_for_status()
                for line in response.iter_lines():
                    if line.startswith("data: "):
