        ...
        try:
            ...
            if not candidates:
                return GenerationResult(
                    content=_t("provider.error.gemini_no_content", lang),
                    citations=[],
            ...
            return GenerationResult(content=content, citations=citations, model_used=model_name, finish_reason="stop")
        except httpx.HTTPStatusError as e:
            lang = getattr(getattr(self, "_local", None), "lang", "vi")
            logger.error(f"Gemini generation failed: {e}")
            detail = (
                " " + _t("error.api_key_invalid", lang)
                if (e.response.status_code == 400 and "API key" in e.response.text)
                else ""
            )
            msg = redact_api_key(e.response.text[:300])
            return GenerationResult(
                content=_t("provider.error.gemini_http", lang, status=e.response.status_code, detail=msg + detail),
                citations=[],
                model_used="gemini/error",
                finish_reason="error",
            )
            ...
            logger.error(f"Gemini generation failed: {e}")
            return GenerationResult(
                content=_t("provider.error.gemini", lang, error=redact_api_key(str(e))),
                citations=[],
                model_used="gemini/error",
                finish_reason="error",
            )
