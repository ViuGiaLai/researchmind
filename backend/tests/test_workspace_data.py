...
from db.migrations import DEFAULT_WORKSPACE_ID, MIGRATIONS, run_migrations
...
def test_migrations_are_idempotent_and_seed_default_workspace(tmp_path):
    ...
    try:
        ...
        versions = session.execute(text("SELECT version FROM schema_migrations ORDER BY version")).all()
        assert versions == [(version,) for version, _name, _migration in MIGRATIONS]
        indexes = {row[1] for row in session.execute(text("PRAGMA index_list('chat_history')"))}
        assert "ix_chat_history_created_at" in indexes
    finally:
        session.close()
        engine.dispose()
