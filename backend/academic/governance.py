...
class GraphExtractionSchema:
    ...
    def prompt(self, input_text: str, entity_types: Iterable[str] | None = None) -> str:
        ...
        relationships = self.tuple_delimiter.join(
...
class AcademicGovernance:
    ...
    def rule_pack_titles(self) -> dict[str, str]:
        ...
        return {pack_id: pack.get("title", pack_id) for pack_id, pack in self._data.get("rule_packs", {}).items()}

    def system_contract(
        self, *, language_instruction: str = "", reasoning_mode: str = "fast", strict_evidence: bool = False
    ) -> str:
        rule_ids = ["evidence_grounding", "citation_integrity", "uncertainty_reporting"]
        if strict_evidence:
            rules = list(self.rules(rule_ids)) + [
                "If the evidence is insufficient, say so; do not use outside knowledge."
            ]
        else:
            rules = list(self.rules(rule_ids))
        presentation = (
    ...
    def perspective_questions_request(self, topic: str, name: str, description: str, focus_areas: str) -> str:
        ...
        return "\n".join(
            [
                f"Perspective: {name}. {description}",
    ...
    def graph_extraction_schema(self) -> GraphExtractionSchema:
        ...
        return GraphExtractionSchema(
            ...
            entity_types=tuple(str(item).upper() for item in data["entity_types"]),
            tuple_delimiter=str(data["tuple_delimiter"]),
            record_delimiter=str(data["record_delimiter"]),
            completion_delimiter=str(data["completion_delimiter"]),
            weight_minimum=float(weight["minimum"]),
            weight_maximum=float(weight["maximum"]),
        )
