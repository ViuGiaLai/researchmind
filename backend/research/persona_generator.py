...
def generate_personas(query: str, related_topics: list[str] | None = None) -> PerspectiveSet:
    ...
    related_context = (
        "\n".join(f"- {topic}" for topic in (related_topics or [])[:5])
        or "No specific reference material is available."
    )
    ...
    try:
        ...
        personas = [
            Persona(
                ...
                focus_areas=[
                    str(area).strip() for area in item.get("focus_areas", []) if isinstance(area, str) and area.strip()
                ][:3],
            )
